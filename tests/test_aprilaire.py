...
class TestAprilAire(unittest.TestCase):
    ...
    def test_get_weather_dehumidifier(self, mock_credentials, mock_id_token, mock_get_json,
        ...
        self.assertTrue(sensor.is_comp_on)
        self.assertFalse(sensor.is_dehum_fan_on)
        self.assertTrue(sensor.is_hvac_fan_on)
        self.assertEqual(sensor.alerts, {"high_hum": False, "low_temp": True})
        self.assertEqual(sensor.fan_time_hours, 44)
        self.assertEqual(sensor.filter_service, {"needs_service": False, "remaining": 100})
